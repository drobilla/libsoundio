...
struct SoundIo {
     ...
     *
     * Called when the backend disconnects.  For example, when the JACK server
     * shuts down.  When this happens, listing devices and opening streams will
     * always fail with SoundIoErrorBackendDisconnected.  This callback is only
     * called during a call to #soundio_flush_events or #soundio_wait_events.
    ...
    /**
     * Optional events signal callback.
     *
     * Called from an unknown thread that you should not use to call any
     * soundio functions.  You may use this to signal a condition variable to
     * wake up.  Called when #soundio_wait_events would be woken up.
     */
     ...
     * Read-only backend.
     *
     * After calling #soundio_connect or #soundio_connect_backend, this field
     * tells which backend is currently connected.
     */
     ...
     *
     * PulseAudio uses this for "application name".  JACK uses this for
     * `client_name`.  Must not contain a colon (":").
     */
    const char *app_name;
     ...
     * default, it will print to stderr only the first time it is called a
     * message instructing the user how to configure their system to allow
...
struct SoundIoDevice {
     ...
     * supports raw mode, there may be up to four devices with the same id: one
     * for each value of #SoundIoDevice::is_raw and one for each value of
     * #SoundIoDevice::aim.
     */
    char *id;
     ...
     * #soundio_sort_channel_layouts on it.  Devices are guaranteed to have at
     * least 1 channel layout.
     */
     ...
     *
     * If it is a raw device, current_format is meaningless; the device has no
     * current format until you open it.  On the other hand, if it is a virtual
     * device, current_format describes the destination sample format that your
     * audio will be converted to.  Or, if you're the lucky first application
     * to open the device, you might cause the current_format to change to your
     * format.  Generally, you want to ignore current_format and use whatever
     * format is most convenient for you which is supported by the device,
     * because when you are the only application left, the mixer might decide
     * to switch current_format to yours.  You can learn the supported formats
     * via formats and #SoundIoDevice::format_count.  If this information is
     * missing due to a probe error, formats will be `NULL`.  If current_format
     * is unavailable, it will be set to #SoundIoFormatInvalid.  Devices are
     * guaranteed to have at least 1 format available.
     */
     ...
     *
     * Sample rate is handled very similar to #SoundIoDevice::formats.  If
     * sample rate information is missing due to a probe error, the field will
     * be set to NULL.  Devices which have #SoundIoDevice::probe_error set to
     * #SoundIoErrorNone are guaranteed to have at least 1 sample rate
     * available.
     */
    struct SoundIoSampleRateRange *sample_rates;

    /**
     * The number of sample rate ranges available in #SoundIoDevice::sample_rates.
     *
     * Set to 0 if sample rate information is missing due to a probe error.
     */
     ...
     *
     * If this value is unknown or irrelevant, it is set to 0.0.  For
     * PulseAudio and WASAPI this value is unknown until you open a stream.
    */
    ...
    */
    ...
    /**
     * Whether this is a raw hardware device.
     *
     * Raw means that you are directly opening the hardware device and not
     * going through a proxy such as dmix, PulseAudio, or JACK.  When you open
     * a raw device, other applications on the computer are not able to
     ...
     *
     * Ideally this will be SoundIoErrorNone in which case all the fields of
     * the device will be populated.  If there is an error code here then
     * information about formats, sample rates, and channel layouts might be
     * missing.
     *
     * Possible errors:
     * - #SoundIoErrorOpeningDevice
...
struct SoundIoOutStream {
     ...
     * actual software latency, as near to this value as possible.  On systems
     * that support clearing the buffer, this defaults to a large latency,
     * potentially upwards of 2 seconds, with the understanding that you will
     * call #soundio_outstream_clear_buffer when you want to reduce the latency
     * to 0.  On systems that do not support clearing the buffer, this defaults
     * to a reasonable lower latency value.
     *
     ...
     * case, the large buffer is there if you want it; you only have to fill as
     * much as you want.  On backends like JACK, `frame_count_min` will be equal
     * to `frame_count_max` and if you don't fill that many frames, you will
     * get glitches.
     *
     * If the device has unknown software latency min and max values, you may
     * still set this, but you might not get the value you requested.  For
     * PulseAudio, if you set this value to non-default, it sets
    ...
    /**
     * Current output volume, from 0 (silent) to 1 (maximum).
     *
     * Used by Core Audio and WASAPI only.
     */
    float volume;
     ...
     * #soundio_outstream_end_write as many times as necessary to write at
     ...
     * until the buffer is full to resume playback.  This is called from the
     ...
     *
     * The `err` parameter is always SoundIoErrorStreaming.
     * SoundIoErrorStreaming is an unrecoverable error.  The stream is in an
     * invalid state and must be destroyed.  If you do not supply
     * error_callback, the default callback will print a message to stderr and
     * then call `abort`.  This is called from the
     * #SoundIoOutStream::write_callback thread context.
     */
     ...
     *
     * Defaults to "SoundIoOutStream".  PulseAudio uses this for the stream
     * name.  JACK uses this for the client name of the client that connects
     * when you open the stream.  WASAPI uses this for the session display
     ...
     *
     * This is used by JACK and it means that the output stream data originates
     * from an input stream.  Defaults to `false`.
     */
    bool non_terminal_hint;

    /**
     * The number of bytes per frame (all channels).
     *
    ...
    /**
     * The number of bytes per sample (one channel).
     *
     * Computed automatically when you call #soundio_outstream_open.
     */
...
struct SoundIoInStream {
     ...
     * actual software latency, as near to this value as possible.  A higher
     * value means less CPU usage.  Defaults to a large value, potentially
     * upwards of 2 seconds.  If the device has unknown software latency min
     * and max values, you may still set this, but you might not get the value
     * you requested.  For PulseAudio, if you set this value to non-default, it
     ...
     * frames.
     *
     * If you return from read_callback without having read `frame_count_min`,
     * the frames will be dropped. `frame_count_max` is how many frames are
     * available to read.
     *
     * The code in the supplied function must be suitable for real-time
     * execution.  That means that it cannot call functions that might block
    ...
    /**
     * This optional callback happens when the sound device buffer is full, yet
     * there is more captured audio to put in it.
     *
     * This is never fired for PulseAudio.  This is called from the
     ...
     * SoundIoErrorStreaming is an unrecoverable error.  The stream is in an
     * invalid state and must be destroyed.  If you do not supply
     * `error_callback`, the default callback will print a message to stderr
     * and then abort().  This is called from the
     * #SoundIoInStream::read_callback thread context.
     */
     ...
     * Optional: Name of the stream.
     *
     * Defaults to "SoundIoInStream".  PulseAudio uses this for the stream
     * name.  JACK uses this for the client name of the client that connects
     * when you open the stream.  WASAPI uses this for the session display
     * name.  Must not contain a colon (":").
     */
...
/// Library version string, like "2.0.0".
SOUNDIO_EXPORT const char *soundio_version_string(void);

/// Library major version as an integer.
SOUNDIO_EXPORT int soundio_version_major(void);

/// Library minor version as an integer.
SOUNDIO_EXPORT int soundio_version_minor(void);

/// Library patch (micro) version as an integer.
 ...
 *
 * You may create multiple instances of this to connect to multiple backends.
 * Sets all fields to defaults.  Returns `NULL` if and only if memory could not
 * be allocated.
 *
 * @see #soundio_destroy
 */
...
/**
 * Try #soundio_connect_backend on all available backends in order.
 *
 * Possible errors:
 ...
 *
 * Note that calling this function merely flips a pointer; the actual work of
 * collecting device information is done elsewhere.  It is performant to call
 * this function many times per second.
 *
 ...
 *
 * Note that if you do not care about learning about updated devices, you might
 * call this function only once ever and never call #soundio_wait_events.
 */
SOUNDIO_EXPORT void soundio_flush_events(struct SoundIo *soundio);

/**
 * Call #soundio_flush_events then block until another event is ready.
 *
 * This will return either when an event is ready, #soundio_wakeup is called,
 * or in unspecified spurious conditions.
 */
SOUNDIO_EXPORT void soundio_wait_events(struct SoundIo *soundio);

/// Unblock #soundio_wait_events.
SOUNDIO_EXPORT void soundio_wakeup(struct SoundIo *soundio);

/**
 * Trigger a device rescan.
 *
 * Normally you will not ever have to call this function, as libsoundio listens
 * to system events for device changes and responds to them by rescanning
 * devices and preparing the new device information for you to be atomically
 * replaced when you call #soundio_flush_events.  However you might run into
 * cases where you want to force trigger a device rescan, for example if an
 * ALSA device has a #SoundIoDevice::probe_error.
 *
 * After you call this you still have to use #soundio_flush_events or
 * #soundio_wait_events and then wait for the #SoundIo::on_devices_change
 * callback.
 *
...
/**
   @}
   @defgroup channel_layouts Channel Layouts
   @{
*/

/**
 * Check whether two channel layouts are equal.
 *
 * Two channel layouts are equal if they have the same number of channels, and
 * each channel ID matches.
 */
SOUNDIO_EXPORT bool soundio_channel_layout_equal(
        const struct SoundIoChannelLayout *a,
        const struct SoundIoChannelLayout *b);

/// Get the user-friendly UTF-8 encoded name of a channel.
SOUNDIO_EXPORT const char *soundio_get_channel_name(enum SoundIoChannelId id);

/**
 * Get the channel ID for a channel name.
 *
 * @param str UTF-8 encoded text which is the name of a channel such as "Front
 * Left", "FL", or "front-left".
 *
 * @param str_len The length of `str` in bytes.
 *
 * @return the corresponding SoundIoChannelId, or #SoundIoChannelIdInvalid if
 * no such channel is found.
 */
...
/**
 * Get the index of `channel` in `layout`.
 *
 * @return An index from 0 to #SoundIoChannelLayout::channel_count in `layout`,
 * or -1 if no such channel is found.
 */
...
/**
 * Set the #SoundIoChannelLayout::name if the layout matches a builtin one.
 *
 * @return Whether a matching builtin layout was found.
 */
SOUNDIO_EXPORT bool soundio_channel_layout_detect_builtin(struct SoundIoChannelLayout *layout);

/**
 * Get the first channel layout in `preferred_layouts` which matches one in
 * `available_layouts`.
 *
 * @return A pointer to the first available layout in `preferred_layouts`, or
 * NULL if none match.
 */
 ...
 *
 * This sorts by descending channel count.
 */
SOUNDIO_EXPORT void soundio_sort_channel_layouts(struct SoundIoChannelLayout *layouts, int layout_count);

/**
   @}
   @defgroup sample_formats Sample Formats
   @{
*/

/**
 * Get the number of bytes per sample (one channel).
 *
 * @return The number of bytes in one sample, or -1 if `format` is invalid.
 */
 ...
 *
 * @return The number of bytes in an entire frame, or -1 if `format` is invalid.
 */
...
/**
 * Get the number of bytes per second (all channels).
 *
 * @param sample_rate The sample rate in Hz (frames per second).
 */
...
/// Get a string representation of a #SoundIoFormat.
...
/**
   ...
   When you call #soundio_flush_events, a snapshot of all device state is saved
   and these functions merely access the snapshot data.  When you want to check
   for new devices, call #soundio_flush_events.  Or you can call
   #soundio_wait_events to block until devices change.  If an error occurs
   scanning devices in a background thread, #SoundIo::on_backend_disconnect is
 ...
 *
 * @return The number of input devices, or -1 if #soundio_flush_events hasn't
 * been called.
 */
 ...
 *
 * @return The number of output devices, or -1 if #soundio_flush_events hasn't
 * been called.
 */
 ...
 *
 * @return A pointer to the input device, or NULL if #soundio_flush_events
 * hasn't been called or `index` is invalid.
 */
 ...
 *
 * @return The index of the default input device, or -1 if there are no devices
 * or #soundio_flush_events hasn't been called.
 */
...
/// Increment the reference count of `device`.
SOUNDIO_EXPORT void soundio_device_ref(struct SoundIoDevice *device);

/// Decrement the reference count of `device` and clean it up if necessary.
SOUNDIO_EXPORT void soundio_device_unref(struct SoundIoDevice *device);

/**
 * Check whether two devices are equal.
 *
 * Two devices are equal if they have the same #SoundIoDevice::id,
 * #SoundIoDevice::is_raw, and #SoundIoDevice::aim.
 */
...
/**
 * Sort the channel layouts of a device.
 *
 * This sorts by descending channel count.
 */
...
/**
 * Get whether `format` is supported by a device.
 *
 * This is a simple convenience function that searches #SoundIoDevice::formats.
 */
SOUNDIO_EXPORT bool soundio_device_supports_format(struct SoundIoDevice *device,
        enum SoundIoFormat format);

/**
 * Get whether `layout` is supported by a device.
 *
 * This is a simple convenience function that searches #SoundIoDevice::layouts.
 */
SOUNDIO_EXPORT bool soundio_device_supports_layout(struct SoundIoDevice *device,
        const struct SoundIoChannelLayout *layout);

/**
 * Get whether a given sample rate is supported by a device.
 *
 * This is a simple convenience function that searches #SoundIoDevice::sample_rates.
 */
SOUNDIO_EXPORT bool soundio_device_supports_sample_rate(struct SoundIoDevice *device,
        int sample_rate);

/// Get the available rate nearest to `sample_rate`, rounding up.
SOUNDIO_EXPORT int soundio_device_nearest_sample_rate(struct SoundIoDevice *device,
        int sample_rate);

/**
   @}
   @defgroup output Output Streams
   @{
*/
 ...
 *
 * This function allocates memory and sets defaults.  Next you should fill out
 * the struct fields and then call #soundio_outstream_open.  Sets all fields to
 * defaults.  Returns `NULL` if and only if memory could not be allocated.
 *
 * @see #soundio_outstream_destroy
 */
 ...
 *
 * You may not call this function from #SoundIoOutStream::write_callback.
 *
 * @see #soundio_outstream_create
 */
SOUNDIO_EXPORT void soundio_outstream_destroy(struct SoundIoOutStream *outstream);

/**
 * Open an output stream with the configured parameters.
 *
 * After you call this function, #SoundIoOutStream::software_latency is set to
 * the correct value.
 *
 * The next thing to do is call #soundio_outstream_start.  If this function
 * returns an error, the outstream is in an invalid state and you must call
 * #soundio_outstream_destroy on it.
 *
 * Possible errors:
 *
 * - #SoundIoErrorInvalid
 ...
 *
 *   - #SoundIoOutStream::layout has a #SoundIoChannelLayout::channel_count
 *     greater than #SOUNDIO_MAX_CHANNELS
 *
 * - #SoundIoErrorNoMem
 *
 * - #SoundIoErrorOpeningDevice
 ...
 * - #SoundIoErrorNoSuchClient - when JACK returns `JackNoSuchClient`
 *
 * - #SoundIoErrorIncompatibleBackend - #SoundIoOutStream::layout has a
 *   #SoundIoChannelLayout::channel_count greater than the number of channels
...
/**
 * Begin writing by acquiring pointers to output channels.
 *
 * Call this function when you are ready to begin writing to the device buffer.
 *
 * @param[in] outstream The output stream you want to write to.
 *
 * @param[out] areas The memory addresses you can write data to, one per
 * channel.  It is OK to modify the pointers if that helps you iterate.
 *
 * @param[in,out] frame_count frame_count Provide the number of frames you want
 * to write.
 *
 * @return The number of frames you can actually write, which is also the
 * number of frames that will be written when you call
 * #soundio_outstream_end_write.  The value returned will always be less than or
 ...
 * #SoundIoOutStream::write_callback.  You must call this function only from
 * the #SoundIoOutStream::write_callback thread context.  After calling this
...
/**
 * End writing output channels.
 *
 * This commits the write that began with a call to
 * #soundio_outstream_begin_write.
 *
 * You must call this function only from the #SoundIoOutStream::write_callback
 * thread context.
 *
 * Possible errors:
 * - #SoundIoErrorStreaming
 ...
 *
 * This function can be called from any thread.  This function can be called
 * regardless of whether the outstream is paused or not.  Some backends do not
 * support clearing the buffer.  On these backends this function will return
 * SoundIoErrorIncompatibleBackend.  Some devices do not support clearing the
 * buffer.  On these devices this function might return
 * SoundIoErrorIncompatibleDevice.
 *
 * Possible errors:
 * - #SoundIoErrorStreaming
 ...
 * the buffer is not full.  Pausing might put the hardware into a low power
 * state which is ideal if your software is silent for some time.  This
 * function may be called from any thread context, including
 * #SoundIoOutStream::write_callback.  Pausing when already paused or unpausing
 * when already unpaused has no effect and returns #SoundIoErrorNone.
 *
 * Possible errors:
 *
 * - #SoundIoErrorBackendDisconnected
...
/**
 * Get the total latency of an output stream.
 *
 * This is the total number of seconds that the next frame written after the
 * last frame written with #soundio_outstream_end_write will take to become
 * audible.
 *
 * This includes both software and hardware latency.  In other words, if you
 * call this function directly after calling #soundio_outstream_end_write, this
 * gives you the number of seconds that the next frame written will take to
 * become audible.
 *
 * This function must be called only from within
...
/**
 * Set the current output volume, from 0 (silent) to 1 (maximum).
 *
 * Used by Core Audio and WASAPI only.
 */
 ...
 *
 * This function allocates memory and sets defaults.  Next you should fill out
 * the struct fields and then call #soundio_instream_open.  Sets all fields to
 * defaults.  Returns `NULL` if and only if memory could not be allocated.
 *
 * @see #soundio_instream_destroy
 */
...
/**
 * Open an input stream with the configured parameters.
 *
 * After you call this function, #SoundIoInStream::software_latency is set to
 * the correct value.
 *
 * The next thing to do is call #soundio_instream_start.  If this function
 * returns an error, the instream is in an invalid state and you must call
 * #soundio_instream_destroy on it.
 *
 * Possible errors:
 * - #SoundIoErrorInvalid
...
/**
 * Begin reading by acquiring pointers to input channels.
 *
 * Call this function when you are ready to begin reading from the device
 * buffer.
 *
 * @param[in] instream The input stream you want to read from.
 *
 * @param[out] areas The memory addresses you can read data from.  It is OK to
 *   modify the pointers if that helps you iterate.  There might be a "hole" in
 *   the buffer.  To indicate this, `areas` will be `NULL` and `frame_count`
 *   tells how big the hole is in frames.
 *
 * @param[in,out] frame_count Provide the number of frames you want to read;
 * returns the number of frames you can actually read.  The returned value will
 * always be less than or equal to the provided value.  If the provided value
 * is less than `frame_count_min` from #SoundIoInStream::read_callback this
 * function returns with #SoundIoErrorInvalid.
 *
 * It is your responsibility to call this function no more and no fewer than
 * the correct number of times according to the `frame_count_min` and
 * `frame_count_max` criteria from #SoundIoInStream::read_callback.  You must
 * call this function only from the #SoundIoInStream::read_callback thread
 * context.  After calling this function, read data from `areas` and then use
 * #soundio_instream_end_read` to actually remove the data from the buffer and
 * move the read index forward. #soundio_instream_end_read should not be called
 * if the buffer is empty (`frame_count` == 0), but it should be called if
 * there is a hole.
 *
 * Possible errors:
 *
 * - #SoundIoErrorInvalid - `*frame_count` < `frame_count_min` or `*frame_count` > `frame_count_max`
 *
 * - #SoundIoErrorStreaming
 *
 * - #SoundIoErrorIncompatibleDevice - in rare cases it might just now be
 *   discovered that the device uses non-byte-aligned access, in which case
...
/**
 * End reading input channels.
 *
 * This will drop all of the frames from when you called
 * #soundio_instream_begin_read.  You must call this function only from the
 * #SoundIoInStream::read_callback thread context.  You must call this function
 * only after a successful call to #soundio_instream_begin_read.
 *
 * Possible errors:
 * - #SoundIoErrorStreaming
 */
...
/**
 * Pause an input stream.
 *
 * If the underlying device supports pausing, this pauses the stream and
 * prevents #SoundIoInStream::read_callback from being called.
 *
 * Otherwise this returns #SoundIoErrorIncompatibleDevice.  This function may
 * be called from any thread.  Pausing when already paused or unpausing when
 * already unpaused has no effect and always returns #SoundIoErrorNone.
 *
 * Possible errors:
 *
 * - #SoundIoErrorBackendDisconnected
...
/**
 * Get the total latency of an input stream.
 *
 * This is the total number of seconds that the next frame of sound being
 ...
 *
 * libsoundio ring buffers use memory mapping techniques to enable a contiguous
 * buffer when reading or writing across the boundary of the ring buffer's
 * capacity.  `requested_capacity` in bytes.  Returns `NULL` if and only if
 * memory could not be allocated.  Use #soundio_ring_buffer_capacity to get the
 * actual capacity, which might be greater for alignment purposes.
 *
 * @see #soundio_ring_buffer_destroy
 */
 ...
 *
 * @see #soundio_ring_buffer_create
 */
